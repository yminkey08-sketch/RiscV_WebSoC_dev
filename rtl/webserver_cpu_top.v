...
module webserver_cpu_top #(
  ...
  localparam init_addr_depth = 1024 * 2;
  ...
  generate
    if (sim_mod == 1) begin : clk_sim
      // 仿真: PLL 旁路, 所有时钟 = 50MHz
      pll_50m #() u_pll (
          .inclk0 (clk_50m_in),
          .c0     (clk_50m),
          .c2     (clk_125m),
    ...
    end else begin : clk_real
      // 真实硬件: MMCM 5路输出 (50/100/125/200/125_90)
      mmcm_50_125 u_pll (
          .clk_50m      (clk_50m_in),
          .clk_125m     (clk_125m),
          .clk_200m     (clk_200m),
  ...
  assign cpu_rd_empty    = fpga_cpu_rd_empty[0];
  assign cpu_rd_reop_pre = fpga_cpu_rd_reop_pre[0];
  assign cpu_rd_rpkt_len = fpga_cpu_rd_rpkt_len[CPU_BUF_ADDR_WIDTH:0];
  assign cpu_rd_rdata    = fpga_cpu_rd_rdata[CPU_BUF_DATA_WIDTH-1:0];
  assign cpu_rd_raddr    = fpga_cpu_rd_raddr[CPU_BUF_ADDR_WIDTH-1:0];
  assign cpu_rd_ren      = fpga_cpu_rd_ren[0];
  assign cpu_wr_full     = fpga_cpu_wr_full[0];
