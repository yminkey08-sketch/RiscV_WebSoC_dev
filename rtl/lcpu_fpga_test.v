...
module lcpu_fpga_test(
  ...
  // MDIO sub-bus (forwarded to lcpu_mdio)
  output reg        SUBBUS_mdio_Req,
  output reg        SUBBUS_mdio_RhWl,
  output reg [11:0] SUBBUS_mdio_ReqAddr,
  output reg [31:0] SUBBUS_mdio_DataWr,
  input      [31:0] SUBBUS_mdio_RdData,
  input             SUBBUS_mdio_Ack,
  ...
  // MDIO sub-bus: forward requests at 0x1000-0x1FFF
  always @ (posedge clk or negedge rst_n) begin 
    if(!rst_n) begin 
      SUBBUS_mdio_Req <= 1'b0;
      SUBBUS_mdio_RhWl <= 1'b1;
      SUBBUS_mdio_ReqAddr <= 12'b0;
      SUBBUS_mdio_DataWr <= 32'b0;
    end
    else begin 
      if(address >= 32'h1000 && address <= 32'h1fff)begin 
        SUBBUS_mdio_Req <= req;
      end
      SUBBUS_mdio_RhWl <= rhwl;
      SUBBUS_mdio_ReqAddr <= address[11:0];
      SUBBUS_mdio_DataWr <= wdata;
    end
  end
  ...
  always @ (posedge clk or negedge rst_n) begin 
    if(!rst_n) begin 
      ...
      else if(SUBBUS_mdio_Ack)rdata <= SUBBUS_mdio_RdData;
      ack <=timeout_ack | reg_ack |program_ram_sb_ack | SUBBUS_mdio_Ack;
